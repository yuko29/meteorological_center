...
menu.addEventListener("click", function () {
  ...
  const menuState = menu.classList.contains("is-active")
    ? "active"
    : "inactive";
  const menuLinksState = menuLinks.classList.contains("active")
    ? "active"
    : "inactive";
  const url =
    "/toggle_menu?menu_state=" +
    menuState +
    "&menu_links_state=" +
    menuLinksState;
...
// 添加點擊事件監聽器
hsinchuLink.addEventListener("click", function (event) {
  handle_click(event, "/hsinchu");
});
taichungLink.addEventListener("click", function (event) {
  handle_click(event, "/taichung");
});
tainanLink.addEventListener("click", function (event) {
  handle_click(event, "/tainan");
});

function update_data(data) {
  // 獲取需要更新的元素
  const electricityValueElement = document.getElementById("electricity_value");
  const electricityMaximumElement = document.getElementById(
    "electricity_maximum"
  );
  const earthquakeTimeElement = document.getElementById("earthquake_time");
  const earthquakeMagnitudeElement = document.getElementById(
    "earthquake_magnitude"
  );
  const reservoirName1Element = document.getElementById("reservoir_name_1");
  const reservoirPercentage1Element = document.getElementById(
    "reservoir_percentage_1"
  );
  const reservoirName2Element = document.getElementById("reservoir_name_2");
  const reservoirPercentage2Element = document.getElementById(
    "reservoir_percentage_2"
  );
  const reservoirName3Element = document.getElementById("reservoir_name_3");
  const reservoirPercentage3Element = document.getElementById(
    "reservoir_percentage_3"
  );
